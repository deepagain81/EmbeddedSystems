...
// constants
const esos_sensor_ch_t PERIFERAL_TO_USE = ESOS_SENSOR_CH03; // temperature sensor
...
ESOS_USER_TASK(user_menu) {
	...
	esos_uiF14_flashLED3(250); // flash the led with a period of 250ms
							   ...
							   // print instructions
	ESOS_TASK_WAIT_ON_SEND_STRING("t4_sensor3.c\n");
	ESOS_TASK_WAIT_ON_SEND_STRING("Get an immediate sample in hex with SW1. Get repeated sampling in Celsius with SW2. Configure repeating sampling using SW3.\n");
	// start user loop
	while (1) {
...
ESOS_USER_TASK(sample_ADC) {
	...
	while (1) {
		...
		if (sampling_mode == 0) {
			...
			sensor_value /= 3; // into the 3V range
			temperature = 100000 * ((int32_t)sensor_value - 424) / 625; // should produce Celsius degrees; 6.25mV per Celsius degree - multiply by 100 to avoid floats - extra 3 places for decimal conversion; offset of 424mV at 0 Celsius
			if (temperature < 0) {
				ESOS_TASK_WAIT_ON_SEND_UINT8('-');
